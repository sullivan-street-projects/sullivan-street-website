...
const MARGIN = { top: 24, right: 72, bottom: 32, left: 40 };
const SVG_W = 960;
const SVG_H = 320;
...
// Simplified Y ticks — WSJ uses clean round numbers, units in subtitle
...
export default function ChartWSJBatonPass() {
  ...
  return (
    <div ref={containerRef} className="w-full max-w-[960px] mx-auto px-6 py-16 md:py-24">
      {/* Serif headline — small, italic, the chart is the hero */}
      <h2
        className="text-[15px] md:text-[17px] italic text-charcoal mb-8 md:mb-10"
      ...
      <div className="relative">
        ...
        >
          ...
          {/* Data line — ~2px per WSJ, smooth monotone cubic curve */}
          <motion.path
            d={linePath}
            fill="none"
            stroke="#1a1a1a"
          ...
          {/* Direct end-of-line label — WSJ convention: dot + value */}
          <motion.g
            initial={reducedMotion ? false : { opacity: 0 }}
            animate={shouldAnimate ? { opacity: 1 } : reducedMotion ? { opacity: 1 } : undefined}
      ...
      {/* Typewriter caption — the baton pass: line draws, then text types */}
