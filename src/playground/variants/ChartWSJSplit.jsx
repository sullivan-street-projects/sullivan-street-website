...
const MARGIN = { top: 8, right: 52, bottom: 32, left: 40 };
const SVG_W = 560;
const SVG_H = 320;
...
// Fewer ticks for narrower chart
const Y_TICKS = [0, 0.5, 1, 1.5, 2];
const Y_VALUES = [0, 500, 1000, 1500, 2000];
const X_TICKS = [2014, 2018, 2022, 2026];
...
export default function ChartWSJSplit() {
  ...
  return (
    <div ref={containerRef} className="w-full max-w-[1200px] mx-auto px-6 py-16 md:py-24">
      <div className="md:grid md:grid-cols-2 md:gap-12">
        {/* LEFT column — big number + subtitle + typewriter */}
        <div className="flex items-center mb-12 md:mb-0">
          <div>
            <h2
              className="text-[80px] md:text-[120px] leading-[0.85] tracking-tight text-charcoal"
              style={{ fontFamily: SERIF }}
            >
              $<AnimatedCounter reducedMotion={reducedMotion} inView={inView} />B+
            </h2>
            <p
              className="mt-3 text-body md:text-body-lg text-muted italic"
            ...
            <div className="mt-6 min-h-[2rem]">
              {reducedMotion ? (
                ...
                <p
                  className="text-body-sm text-muted relative"
                  style={{ fontFamily: SANS }}
                >
        ...
        <div>
          ...
          >
            <defs>
              <linearGradient id="wsjSplitAreaGrad" x1="0" y1="0" x2="0" y2="1">
            ...
            {/* X-axis labels — fewer ticks for narrower chart */}
            {X_TICKS.map((year) => (
              <text
                key={`xl-${year}`}
                x={xPos(year)}
            ...
            {/* Area fill — ~12% opacity */}
            <motion.path
              d={areaPath}
              fill="url(#wsjSplitAreaGrad)"
            ...
            {/* Data line — ~2px, smooth monotone cubic */}
            <motion.path
              d={linePath}
              fill="none"
              stroke="#1a1a1a"
          ...
          {/* Source line — WSJ convention: small, gray */}
          <p
            className="mt-3 text-caption text-faint"
            style={{ fontFamily: SANS }}
          >
