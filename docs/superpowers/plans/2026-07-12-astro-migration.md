...
- A step needs a credential/token that isn't present — ask the operator; never fabricate. If the step is explicitly marked DEFERRABLE (Task 15's verification tokens), record it as deferred and continue instead of halting.
...
**Note:** The CookieConsent island is deliberately absent from this file until Task 10 — `CookieConsent.jsx` still imports `react-router-dom` at this point, and importing it here would break the build. Task 10 Step 3 adds the import and replaces the placeholder comment. Copy this block exactly as written.
...
(Verified against source: `TermsConditions.jsx:5` passes `title="Terms &amp; Conditions"` — a JSX-escaped ampersand that renders as "Terms & Conditions". In Astro, write the prop as the plain string `title="Terms & Conditions"`, exactly as the shell above already does. Also verified: neither legal page contains any `<Link>` — conversion recipe item 3 applies zero times.)
...
- [ ] **Step 2: 🔑 Obtain verification tokens — DEFERRABLE (does not block the loop)**

Ask the operator for the two tokens. Google Search Console (`search.google.com/search-console`) → add property `sullivanstreetprojects.com` → choose **HTML tag** method → copy the `content` token. Bing Webmaster (`www.bing.com/webmasters`) → add site → **Meta tag** → copy the `content` token. (Bing can also **import** the GSC property, which skips its own verification — prefer that if offered.)

**If the operator is unavailable:** leave both tokens as `''` in Step 3, record this step as deferred, and continue the loop. Filling the tokens in any later session auto-activates the Step 1 checks.

- [ ] **Step 3: ⚙️ Add verification meta tags to `src/layouts/BaseLayout.astro`**

In the frontmatter, add (paste the Step 2 values when available; `''` = tag omitted from HTML and its check skipped):

```astro
// Search-engine verification tokens (Google Search Console + Bing Webmaster).
// '' = not yet collected; verify-dist.mjs enforces the meta tag once set.
const SITE_VERIFICATION = {
  google: '', // 🔑 GSC HTML-tag content token
  bing: '', // 🔑 Bing meta content token
};
```
