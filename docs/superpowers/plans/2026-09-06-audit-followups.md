...
Verified after all tasks complete. Criteria 1–14 gate the human checkpoint; 15–19 are checked after the deploy in Task 11; 20–24 were appended by verify-plan's impact analysis on 2026-09-06.
...
| 20  | [impact] `--color-label` now equals `--color-faint` (#6b6b6b) — intentional per spec C3 ("the palette gains no new grey") | `grep -c -- '#6b6b6b' src/styles/global.css` = 2 and the spec's C3 bullet names the collision                                                                                                                                                                       |
| 21  | [impact] CLAUDE.md tells editors that the founder bio lives in two places (CONTENT.md/About + FOUNDER)                    | `grep -cF 'two-place edit (About.astro + FOUNDER)' CLAUDE.md` = 1                                                                                                                                                                                                   |
| 22  | [impact] CLAUDE.md documents the `/contact` rescue 301, its lack of a file-existence guard, and the query-string caveat   | `grep -cF '/contact` is a rescue 301' CLAUDE.md` = 1                                                                                                                                                                                                                |
| 23  | [impact] A malformed `~/.secrets/accounts.json` fails every script with a clear message, not a stack trace                | `T=$(mktemp -d); mkdir -p $T/.secrets; echo '{ bad' > $T/.secrets/accounts.json; HOME=$T node scripts/gsc.mjs sites 2>&1 \| head -1` starts with `Malformed`                                                                                                        |
| 24  | [impact] The `GSC_SA_KEY`-beats-`--account` precedence is documented in both Google scripts                               | `grep -c 'unset it when switching accounts' scripts/ga.mjs scripts/gsc.mjs` → `1` for each                                                                                                                                                                          |
